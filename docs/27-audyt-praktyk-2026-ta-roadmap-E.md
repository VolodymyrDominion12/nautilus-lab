...
| E-2.4 | vitest + Testing Library для `lib/*` і ключових компонентів; Playwright-smoke (дашборд відкривається, сесії видно, WS підключився) проти `uvicorn` з синтетикою | 🟠 | 🟡 крок 1 2026-09-25: vitest (`npm test`) для `src/lib`: `research-logic.test.ts` — 76 перевірок `preflight()`/`verdictFor()`, перенесених зі скрипта `scripts/check-research-logic.ts` (тепер під `tsc`: знайдено й виправлено spread `undefined` у кейсі «нижче нульової вартості»), `format.test.ts` — 27 тестів «невиміряне лишається невиміряним» (`toNumber`, `formatPct`, `toneOf`, дати, `describeStaleness`, `evidenceBadge`). ✅ крок 2 2026-09-25: Playwright-smoke `npm run e2e` (`frontend/e2e/smoke.e2e.ts`): продакшн-збірка дашборду проти справжнього `create_app` на синтетичних ринках (`scripts/e2e_server.py`: історія й kline-потік із `synthetic_ohlcv`, без мережі, ключів і журналу) — дашборд читає `/api/status` крізь CORS і origin-гейт, сесія видна в таблиці, WS «Stream Active», нові бари приходять, жодної `pageerror`. Серверна частина має власний `pytest` (`test_e2e_server.py`: прогрів, живі бари, WS `INIT_STATE`, чужий origin → 403). `frontend.yml`: `npm test` + окремий job `e2e`. ⏳ Testing Library для компонентів — разом із розбиттям у E-2.5 |
| E-2.5 | Розбити `ResearchLab`, `LiveTradingTerminal`, `CatalogManager`; TanStack Query для запитів/polling | 🔴 | жоден компонент > 15 КБ; polling зупиняється на прихованій вкладці | 🟡 крок 1 2026-09-25: TanStack Query — `services/queries.ts` (ключі й інтервали в одному місці; `refetchIntervalInBackground: false` — прихована вкладка не опитує API; `retry: 1`), `QueryClientProvider` у `main.tsx`; `App` читає status/catalog/strategies через `useQuery` замість `setInterval`, завершений джоб інвалідовує кеш. `App.tsx` 16.5 → 9 КБ (бічна панель → `Sidebar.tsx`, вкладки → `tabs.ts`); `CatalogManager.tsx` 26 → 4 КБ (`catalog/CatalogHeader`, `InstrumentCards`, `SeriesCoverage`, `IngestPanel` — лог інжесту через `useQuery` лише поки джоб іде, з захистом від застарілої відповіді попереднього запуску). Бюджет розміру `src/componentSize.test.ts`: храповик — нові файли ≤ 15 КБ, п’ять ще завеликих (`ResearchLab` 57, `LiveTradingTerminal` 50, `AlphaIdeasTab` 25, `CommandCenter` 23, `EquityCurveChart` 17 КБ) можуть лише зменшуватись. `queries.test.ts` фіксує контракт опитування; Playwright-smoke відкриває вкладку каталогу. ✅ крок 2 2026-09-25: `ResearchLab.tsx` 57 → 13.5 КБ. Форма — одне значення `ResearchForm` (`lib/researchForm.ts`: відновлення з localStorage з дефолтами, пресет, архівний прогін, reset, тіло POST і CLI-команда — чисті функції, `researchForm.test.ts` 17 тестів) замість 26 `useState`; хуки `research/useResearchForm` (збереження, узгодження з робот/гіпотеза/дефолти спеки/інструмент) і `research/useResearchRun` (запуск, лог через `researchLogQuery` лише поки джоб іде, та сама логіка «чужий результат» / «процес помер без результату», захист від кешованої відповіді попереднього джоба); панелі `research/BasicControls`, `RunControls`, `ParamOverrides`, `AdvancedGates`, `ResearchBanners`, `RunConditions`, `TearsheetPanel`. Каталог і покриття серій — зі спільного кешу (той самий запит, що в бічній панелі й на вкладці каталогу). Playwright-smoke відкриває вкладку Research. ✅ крок 3 2026-09-25: `CommandCenter` 23 → 2.7 КБ (`command/JobCards`, `LastResultPanel`, `RecentExperiments`, `JournalSummary`, `DataCoverage`, `SystemPanels`; опитування 5 с через `commandCenterQuery` — зупиняється на прихованій вкладці), `AlphaIdeasTab` 25 → 2 КБ (`alpha/ProposePanel`, `HypothesisArchive` — список і деталі файлу через `hypothesesQuery`/`hypothesisDetailQuery`, `HypothesisCard`), `EquityCurveChart` 17 → 9.6 КБ (арифметика ланцюга фолдів — `lib/equityCurve.ts` + `equityCurve.test.ts`: складні відсотки, просідання від піку, невиміряний фолд не ламає криву; `equity/EquitySummary`, `EquityTooltip`). Бюджет: поза лімітом лишився тільки `LiveTradingTerminal` (50 КБ, його зараз змінює робота над decision log). Playwright-smoke відкриває Command Center і Alpha Ideas. ✅ `LiveTradingTerminal` 50 → 8.5 КБ (паралельна сесія 2026-09-25: `terminal/useLiveTradingSession` + панелі `terminal/*`, `LiveTradingTabs`) — жоден компонент не перевищує 15 КБ. ⏳ тести компонентів (Testing Library) |
| E-2.6 | «Золотий» регресійний бектест: фіксований синтетичний датасет + фіксований seed → знімок ключових метрик у репо; CI порівнює | 🟢 | ✅ код 2026-09-25: `scripts/golden_backtest.py` (5 випадків: regime, ema, adaptive_ema, vpin_momentum на 3000 синтетичних барах + walk-forward regime 2 фолди; налаштування лише з дефолтів коду — ні `.env`, ні змінних оболонки), знімок `tests/golden/backtests.json` з версією NautilusTrader, `tests/integration/test_golden_backtest.py` друкує кожне змінене число. ✅ знімок `tests/golden/backtests.json` створено локально (2026-09-25) |
...
| E-2.9 | Розширити ruff (`S`, `ASYNC`, `DTZ`, `PT`, `C4`, `PERF`, `BLE`); dev-інструменти → `[dependency-groups]` | 🟠 | ✅ правила 2026-09-25: `S`, `ASYNC`, `DTZ`, `PT`, `C4`, `PERF`, `BLE` увімкнено, `ruff check` зелений (112 знахідок → 0). Справжні виправлення: `LLM_BASE_URL` і heartbeat-URL лише http(s) (urllib відкривав би `file:`), `assert` у робочому коді → явні помилки (`pairs_trading._fitted()`, `run_xsmom`), наївні `datetime.now()` → з часовим поясом, 24 складені `assert` у тестах розбито, `pytest.raises` з `match=`. Навмисні межі (`except Exception` у джобах/алертах, фіксований argv у subprocess) позначено `# noqa: <код> — причина`. Файли, які зараз змінює робота над decision log, мають тимчасові per-file-ignores. ✅ `[dependency-groups]` 2026-09-25: патч `e29-dependency-groups.patch` застосовано (pyproject, Makefile, ci.yml `--group dev`, README). Встановлення: `uv sync --all-extras` — без extras зникають fastapi/websockets/yaml |
...
| R-2 | Попередня реєстрація + хеш у маніфесті | 🟠 | §10 DoD | `PROMOTE` неможливий без збігу хешу | ✅ 2026-09-26: `lab research --folds N --register "гіпотеза"` рахує вікна фолдів (`RunWalkForward.plan_multi`, без жодного бектесту) і пише умови тесту з їхнім sha256 у `research/preregistrations/*.json` (ніколи не перезаписується). Умови (`ResearchTerms`): робот, датасет, сітка (або `optuna seed+trials`), фолди, IS-частка, embargo, метрика відбору, точні OOS-вікна, пороги `GateCriteria`, фільтри/комісії/ризик. Звичайний прогін з `--folds N` перераховує умови з того, що зробив, пише хеш у маніфест (`preregistration_sha256`, `terms=` у рядку маніфесту) і журнал, і порівнює: `matched` / `mismatch` (називає, що змінилось) / `registered after the run` / `not registered`; файл, змінений після реєстрації, не рахується. Гейт: перевірка `preregistered` — без збігу PROMOTE неможливий (NOT MEASURED або FAIL). [ADR 0008](adr/0008-poperednia-reiestratsiia-testu.md). `tests/unit/test_preregistration.py` (19), `test_promotion_gate.py` +3 |
| R-3 | Глобальний лічильник спроб для DSR | 🟢 | §10 п.4 | DSR у звіті вказує `n_trials_total` | ✅ 2026-09-26: журнал спроб `research/trials.jsonl` (`TRIALS_LEDGER_PATH`, append-only, рахунок перераховується з усього файлу): кожен PBO-аудит і кожен walk-forward (сітка — мітки конфігурацій, Optuna — `seed + номер спроби`) на **каталожних** даних записує свої конфігурації (`robot|params`) під ключем датасету (`bar_type`, для пари — обидві ноги); синтетика не пишеться. `deflated_sharpe_ratio(..., total_trials=)`: N в очікуваному максимумі = усі спроби на цих даних, дисперсія — з поточного прогону; повторний той самий прогін нічого не додає, відстаючий журнал не може зменшити N. `n_trials_total` — у `summary_line`, у рядку гейта `dsr`, у JSON (`trials_total`) і в підказці панелі PBO. `tests/unit/test_trial_ledger.py` (13 тестів) |
| R-4 | Vol-matched buy & hold у всіх звітах і гейті | 🟢 | §1.2 | колонка в фолд-таблиці | ✅ 2026-09-26: `BacktestMetrics.return_volatility` (σ покарових повернень кривої капіталу), `vol_matched_from_volatility` (buy & hold × σ_робота/σ_активу, плече ≤ 2×, σ = 0 → готівка, невиміряне → None); кожен `WalkForwardFold` несе `vol_matched_buy_and_hold_return`, `MultiWindowReport` — середнє, `beats_vol_matched_buy_and_hold()` і рядок у `summary_line`; JSON (`vol_matched_buy_and_hold_return`, `…_mean`, `beats_vol_matched_buy_and_hold`), колонка «Vol-matched» у таблиці фолдів. Гейт: нова перевірка `beats_vol_matched` поруч із `beats_buy_hold` (обидві обов'язкові; [ADR 0007](adr/0007-buy-hold-za-volatylnistiu-u-heiti.md)). `tests/unit/test_vol_matched_benchmark.py`, `test_promotion_gate.py` +3 |
