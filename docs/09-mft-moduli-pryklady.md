...
Другий дає `sell`, бо евристика бачить від'ємний OBI. `reason` — `ml_obi_hawkes`, бо стратегія
годує кожен знімок ще й у `ExponentialHawkes` і додає інтенсивності потоку в причину сигналу.
...
inventory=  10  bid=3498.24956  ask=3501.74964
inventory= -10  bid=3498.25036  ask=3501.75044
```
...
> **Зверніть увагу на масштаб зсуву.** Ціна резервації в коді —
> `mid − gamma × variance × inventory`, тобто скос пропорційний **дисперсії** (`volatility²`),
> а не самій волатильності. У прикладі передано `volatility=0.02` — це per-bar **дохідність**,
> а не ціна в USDT, тому `variance = 0.0004`, і зсув на інвентарі ±10 виходить ~0.0004 USDT
> (тобто майже нуль поруч зі спредом 3.5 USDT). Клас очікує `volatility` у **тій самій одиниці,
> що й `mid`** — per-bar стандартне відхилення ціни в котирувальній валюті; передайте туди
> 2%-ну дохідність, і складова інвентарю практично зникне.
...
**Вивід (старий — до виправлення ADF, див. нижче):**
...
`hedge ratio`, `half-life` і `поточна Z` тут правильні — вони не залежать від ADF-тесту. А рядок
`ADF p-value: 0.50` — це **вивід самого баґу**, а не результат воріт: стара реалізація порівнювала
сирий коефіцієнт `gamma` з жорстко зашитими межами, а для AR(1) `gamma = rho − 1` замкнений
у `(−2, 0)`, тож межа `gamma < −2.9` була недосяжна і спрацьовувала гілка `else` (саме `0.50`).

Саме з цього рядка раніше робили хибний висновок, що робот `pairs` «не торгує, бо пара
некоінтегрована». Насправді причина була в реалізації тесту. Тепер `fit_cointegration` рахує
справжнє t-відношення коефіцієнта при `e_{t−1}`, обирає лаг за BIC і бере p-value з квантилів
МакКіннона (2010) для `N = 2`, `regression="c"`; обраний лаг видно в полі
`CointegrationResult.adf_lags`. Перезапустіть фрагмент вище — `ADF p-value` буде справжнім числом
для цієї синтетичної пари.

Практичний підсумок: ворота відкриваються, і `pairs` справді торгує — на розширеному каталозі
(IS 2024-01-01 … 2025-11-22, OOS 2025-11-22 … 2026-09-14, старт 100 000) `IS fills=204
ending=99613.88` і `OOS fills=300 ending=98958.10`. Результат трохи **від'ємний на обох вибірках**,
тож це доказ працездатності воріт, а не наявності переваги.

Розбір баґу й доказ правильності проти `statsmodels` —
