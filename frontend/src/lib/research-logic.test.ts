/**
 * Research-discipline rules the dashboard enforces in the UI (docs/27 E-2.4).
 *
 * Ported from `scripts/check-research-logic.ts` (the jiti script) to vitest: the same
 * checks, now run by `npm test` and failing CI like any other test. They cover the rules
 * that must not silently regress — a preflight that stops an impossible run, and a verdict
 * that refuses to read an in-sample number as a result.
 */

import { expect, test } from 'vitest';

import { describeStaleness, formatBps, formatPct, toneOf } from './format';
import {
  cliCommand,
  foldLayout,
  fractionBoundaries,
...
import type { MultiWindowSummary, ResearchSummary, StrategySpec } from '../services/api';

/** One named rule. The condition is computed while the file loads; the test reports it. */
...
// ---- foldLayout — must mirror floor(total * is_fraction) and per_fold division
{
...
// ---- preflight — accepts a run the engine can actually complete
check('a sane catalog run has no blocking error', errorsOf(baseInput).length === 0, errorsOf(baseInput).join(' | '));

// ---- preflight — blocks runs the backend is known to refuse
check(
...
// ---- preflight — warns without blocking
{
...
// ---- preflight — tick-level filters are never accepted without the data behind them
{
  const tickReady = {
    ...baseInput,
    tickVpin: true,
...
// ---- preflight — a stress slice must lie inside the catalog it runs on
{
...
// ---- cliCommand — only flags that exist in interfaces/cli.py
{
...
// ---- format — an unmeasured number is never rendered as zero
...
// ---- staleness — a catalog that ends months ago is not "fresh"
{
...
// ---- fractionBoundaries — derives the bar-index split, not a time fraction
{
...
// ---- verdict — never reads an in-sample number as a result
{
  ...
  const winningWindow: MultiWindowSummary = {
    profitable: '2/2',
    fold_count: 2,
    mean_oos: '+2.00%',
    mean_oos_raw: '0.02',
  ...
  const noHeadroom = verdictFor({ ...summaryBase, multi_window: winningWindow });
