...
2. `--catalog` вказує не туди (у CLI відносний шлях рахується від робочої теки).
...
**Якщо шлях у помилці веде у `frontend/`** (`.../frontend/catalog`), то `ingest` тут ні до чого:
API-сервер стартував не з кореня проєкту. Дашборд рахує відносний `CATALOG_PATH` **від кореня
проєкту**, а не від робочої теки процесу, тож перезапускати треба лише сервер:

```bash
cd /path/to/nautilus-lab        # корінь проєкту, не frontend/
.venv/bin/uvicorn nautilus_lab.api.app:app --port 8000
```

Перевірити, який каталог бачить API: `curl -s localhost:8000/api/status | grep -o '"catalog_path":"[^"]*"'`
має показати шлях у корені проєкту. А що взагалі лежить у каталозі:
...
Мінімум барів залежить від робота (`minimum_bars` у `application/run_research_backtest.py`):

| Робот | Мінімум на фолд |
|-------|------------------|
| `regime`, `vpin_momentum`, `meta_label`, `adaptive_ema` | 150 |
| `pairs` | 200 |
| `formulaic_lgbm` | 80 |
| `ema`, `ml_obi` та решта | 50 |
...
### `robot 'funding' has no backtest adapter yet; use one of: ...`

Так і задумано. Сьогодні до рушія підключено **вісім** роботів
(`BACKTEST_WIRED_ROBOTS` у `domain/regime.py`):

```
adaptive_ema, ema, formulaic_lgbm, meta_label, ml_obi, pairs, regime, vpin_momentum
```

Саме цей перелік код і друкує в повідомленні. Решта назв `RobotName` — `funding`,
`glft`, `tri_scan` — існують лише як доменні модулі й **не** підключені до рушія
бектесту, тому команда завершується помилкою (код виходу 1), а не тихим запуском
іншої стратегії.
...
### У журналі paper-сесії кілька `buy` підряд

Розрізняй дві різні речі в `application/run_paper.py`:

| Що | Стан позиції | Де використовується |
|----|--------------|---------------------|
| `RunPaperSession` | **веде**: кожен філ, кожна позиція, комісії, реалізований і нереалізований PnL, переоцінка відкритої позиції на останньому барі | саме це запускає `lab paper` (через рушій Nautilus) |
| `RunPaperResearch` | **не веде**: рахує лише сигнали, які робот подав би | офлайн-прев'ю в API і smoke-тести |

Тож якщо кілька `buy` підряд видно в журналі **сесії** — це не спрощення, а привід
дивитись уважніше: сесія тримає позицію, і повторний вхід означає, що робот справді
подав новий сигнал (або що позицію закрив стоп/зустрічний сигнал і потім відкрив знову).
Плутанина виникає, коли результат прев'ю (`RunPaperResearch`) подають як результат сесії:
прев'ю відповідає лише на питання «чи робот узагалі подає сигнали».

Якщо ж сесія дає `fills=0` — це найчастіше не баг, а дефолтні параметри: додай
`--select-on-is`, щоб параметри підбиралися на історії **перед** вікном сесії
(розбір — [24 §5](24-paper-treydynh.md)).
...
### `--tick-vpin` / `--hawkes` не падають, але й не фільтрують нічого

Найковазніша пастка тік-рівневих фільтрів. Рушій читає **відсутню** серію тіків як
**порожню** — це прямо задокументовано в `infrastructure/agg_trades_catalog.py::load`
(«Returns an empty list when no data exists for the requested window»). Тому запуск
`lab research --tick-vpin` на каталозі, де `lab ingest --trades` ніколи не виконувався,
**не дає помилки**: фільтр просто не має на чому рахувати, і прогін виглядає
тік-фільтрованим, хоча ним не є.

Лікування — спершу перевірити, що серія існує:

```bash
find catalog/data/agg_trade -maxdepth 2 -type d    # має бути тека з символом
uv run lab ingest --trades --start 2025-01-01 --end 2025-01-08 --symbols ETHUSDT
```

Не плутай три прапорці: `--bar-vpin` рахує VPIN на **обсязі бару** (жодних тіків не
потрібно, це свідомо слабший проксі), `--tick-vpin` і `--hawkes` — на справжніх
агрегованих угодах. Веб-дашборд показує наявність серії (`present`/`missing`) по кожному
інструменту саме тому, що «прогін завершився» не є доказом, що дані під фільтром були.

Тікову історію в дослідницькому масштабі REST не віддає — збирай її живим потоком
(`--trades --live-ticks MINUTES`); виміри — [24 §5.2](24-paper-treydynh.md).

### `LAB_ROLE=paper` не дає запустити research / ingest / ML

Так і задумано. Сервер, який торгує paper 24/7, не повинен уміти переписувати власні
налаштування з браузера. `api/security.py` дозволяє ролі `paper` лише:

```
POST /api/paper/live/start   POST /api/paper/live/stop
POST /api/paper/live/close-position   POST /api/paper/live/update-stops
POST /api/paper/sessions
```

Читання (дашборд, `/api/status`, звіти) лишається відкритим; усе інше — research, ingest,
навчання ML, пропозиції альф і `PUT /api/settings` — відхиляється. Дослідження роби на
робочій станції, де лежать дані й обчислення. Деталі — [26](26-deploy-vps.md).

### Дашборд відкривається, але порожній / «немає даних»

Дашборд — **два процеси**, і UI не має власних даних: спершу API, потім фронтенд.

```bash
.venv/bin/uvicorn nautilus_lab.api.app:app --port 8000   # потрібен extra api
cd frontend && npm install && npm run dev                # :5173
```

Якщо API не піднявся — перевір, чи встановлено extra: `uv sync --extra api` (fastapi,
uvicorn, websockets, pyyaml, python-dotenv). Якщо браузер скаржиться на походження
