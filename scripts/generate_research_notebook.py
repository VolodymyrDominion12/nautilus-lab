    ...
    "cells": [
        ...
        },
        {
            ...
            "source": [
                ...
                'print(f"✓ Режим торгівлі: {cfg.trading_mode} (live fail-closed by design)")',
                "\n",
                "# Параметри демонстрації. Типові значення — повний прогін;\n",
                "# CI перекриває їх меншими, щоб гейт виконував зошит за хвилини.\n",
                'DEMO_BARS = int(os.environ.get("LAB_NOTEBOOK_BARS", "3000"))\n',
                'DEMO_FOLDS = int(os.environ.get("LAB_NOTEBOOK_FOLDS", "4"))\n',
                'DEMO_BLOCKS = int(os.environ.get("LAB_NOTEBOOK_BLOCKS", "8"))',
            ],
        },
        {
            "cell_type": "markdown",
        ...
        },
        {
            ...
            "source": [
                ...
                '        print(f"✓ У каталозі знайдено {len(bars)} барів для {cfg.bar_type}")\n',
                "        if bars:\n",
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "# Доменні індикатори — потокові класи над закритими барами (OhlcvBar, Decimal),\n",
                "# а не функції над масивами numpy: саме тому заглянути в майбутнє неможливо.\n",
                ...
                "        open=Decimal(str(close_prices[i])),\n",
                "        high=Decimal(str(high_prices[i])),\n",
                "        low=Decimal(str(low_prices[i])),\n",
                ...
                'vpin = BarVpin(bucket_volume=Decimal("1000"), toxic_threshold=Decimal("0.7"))\n',
                "breakout = UptrendBreakout(\n",
                "    instrument_id=str(cfg.instrument_id), channel_period=20, ema_period=40\n",
                ")\n",
                "\n",
                "for demo_bar in demo_bars:\n",
                "    atr.update(demo_bar)\n",
                "    vpin.update(demo_bar)\n",
                "    breakout.on_bar(demo_bar)\n",
                "\n",
                'print(f"Згенеровано {n_bars} тестових барів.")\n',
                'print(f"Останній ATR(14): {atr.value}")\n',
                "vpin_state = vpin.last\n",
                'vpin_display = vpin_state.value if vpin_state else "немає даних"\n',
                'print(f"Останній VPIN: {vpin_display} (токсичність потоку)")',
            ],
        },
        {
            "cell_type": "markdown",
        ...
        },
        {
            ...
            "source": [
                ...
                "    bar_count=DEMO_BARS,\n",
                '    in_sample_fraction=Decimal("0.7"),  # Decimal, не float: домен не приймає float\n',
                "    folds=1,\n",
                ")\n",
                "\n",
                ...
                'print(f"Параметри, обрані на In-Sample: {wf_result.selected}")\n',
                'print(f"In-Sample баланс:  {wf_result.in_sample.ending_balance:.2f} (fills={wf_result.in_sample.fills})")\n',
                'print(f"★ Out-of-Sample баланс (ЗВІТНИЙ): {wf_result.out_of_sample.ending_balance:.2f} (fills={wf_result.out_of_sample.fills})")\n',
                'print(f"OOS PnL: {window_return(wf_result.out_of_sample, req.backtest.starting_equity):+.2%}")',
            ],
        },
        {
            "cell_type": "markdown",
        ...
        },
        {
            ...
            "source": [
                ...
                '    print(f"  Fold {i}: OOS={fold.oos_return if fold.oos_return is not None else Decimal(0):+.2%} (fills={fold.out_of_sample.fills})")',
            ],
        },
        {
            "cell_type": "markdown",
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "pbo_req_obj = overfit_audit_request(\n",
                "    cfg,\n",
                "    robot=robot_choice,\n",
                "    source=BarOrigin.SYNTHETIC,\n",
                "    bar_count=DEMO_BARS,\n",
                ...
                ")\n",
                "\n",
                "pbo_runner = overfit_audit_use_case(cfg)\n",
                ...
                'print(f"Кількість симетричних сплітів: {pbo_audit.split_count}")\n',
        ...
        },
        {
            ...
            "source": [
                ...
                "for slice_name, stress_slice in STRESS_SLICES.items():\n",
                "    print(\n",
                '        f"  • {slice_name.value}: "\n',
                '        f"{stress_slice.start:%Y-%m-%d} … {stress_slice.end:%Y-%m-%d} — "\n',
                '        f"{stress_slice.description}"\n',
                "    )",
            ],
        },
        {
        ...
        },
        {
            ...
            "source": [
                ...
                "# Пишемо в ТИМЧАСОВИЙ журнал: повторний запуск зошита не має\n",
                "# змінювати відстежуваний research/journal.md\n",
                'journal_dir = Path(tempfile.mkdtemp(prefix="nautilus-lab-journal-"))\n',
                'markdown_path = journal_dir / "journal.md"\n',
                'jsonl_path = journal_dir / "journal.jsonl"\n',
                "markdown_path.write_text(\n",
                '    "# Journal\\n\\n| Дата | Джерело | Гіпотеза | Ворота | OOS | buy&hold | Рішення | Причина |\\n"\n',
                '    "|---|---|---|---|---|---|---|---|\\n"\n',
                '    f"{ROWS_START}\\n{ROWS_END}\\n",\n',
                '    encoding="utf-8",\n',
                ")\n",
                ...
                "    decision=PENDING,\n",
                '    reason=f"Profitable in {multi_res.profitable_folds}/{len(multi_res.folds)} folds, PBO={pbo_audit.pbo:.2f}",\n',
                "    oos_return=multi_res.mean_oos_return,\n",
                "    buy_and_hold_return=multi_res.mean_buy_and_hold_return,\n",
                "    fills=multi_res.total_oos_fills,\n",
                ")\n",
                "record_run(markdown_path=markdown_path, jsonl_path=jsonl_path, entry=entry)\n",
                "\n",
                'print(f"Записано рядок рішення у {markdown_path} та {jsonl_path}")\n',
                'print(f"| {entry.created_at:%Y-%m-%d} | {entry.source} | {entry.subject} | {entry.gates} | {entry.decision} |")',
            ],
        },
        {
            "cell_type": "markdown",
