...
### `--robot funding`, `--robot ml_obi`, `--robot glft`, `--robot tri_scan` дають той самий результат, що `regime`

Це **очікувано**: перелічені модулі не підключені до рушія, і `_build_robot()` для будь-якого
невідомого імені повертає `RegimeRouter`. Деталі та приклади — [05 §0](05-roboty.md#0-таблиця-стану-читати-першою).
