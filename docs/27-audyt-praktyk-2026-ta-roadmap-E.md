...
| E-2.8 | Одна згенерована таблиця стану (`docs/STATUS.md` зі спек + `BACKTEST_WIRED_ROBOTS` + `GateCriteria`); `docs/21/22` посилаються на неї замість дублювання; ADR-тека для рішень | 🟠 | валідатор падає, якщо `STATUS.md` застарів; пороги гейта мають одне джерело |
