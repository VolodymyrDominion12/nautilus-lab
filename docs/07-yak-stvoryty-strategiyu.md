...
| 3.1–3.3 Pairs trading | `pairs/cointegration.py`, `pairs/ou.py`, `pairs/pairs_trading.py` | Замінити спрощений ADF на справжній, динамічний хедж-коефіцієнт | 🟠 середньо |
...
Поточна реалізація (`pairs/cointegration.py`) використовує грубий ADF, через що `pairs` взагалі
не торгує (див. [05](05-roboty.md#3-pairs--статистичний-арбітраж-дві-ноги)). Найкорисніша
доопрацювання в проєкті — замінити його.

```bash
uv add statsmodels        # або: uv add --optional research statsmodels
```

```python
# domain/pairs/cointegration.py
from statsmodels.tsa.stattools import coint          # type: ignore[import-untyped]


def fit_cointegration(y: tuple[Decimal, ...], x: tuple[Decimal, ...]) -> CointegrationResult:
    ...
    # справжній Engle-Granger: p-value з таблиць MacKinnon
    stat, pvalue, _crit = coint([float(v) for v in y], [float(v) for v in x])
    return CointegrationResult(
        hedge_ratio=beta,
        intercept=alpha,
        adf_statistic=Decimal(str(stat)),
        adf_pvalue=Decimal(str(pvalue)),
        residual_std=_std(residuals),
    )
```

Що зміниться:
- p-value стане статистично осмисленим, і ворота `adf_pvalue_max=0.05` почнуть пропускати
  пари, які справді коінтегровані;
- `coint()` повертає p-value, тому тест `test_cointegration_and_ou_on_synthetic_pair` може
  почати падати на синтетиці — це нормально, синтетична пара має слабке повернення;
- `float`-конверсія на межі домену — компроміс: `statsmodels` працює тільки з float.
  Внутрішні розрахунки (розмір, спред) лишаються `Decimal`.

Обов'язково після зміни: `uv run pytest`, потім `uv run lab research --robot pairs`.
Якщо `fills` стане > 0 — ви щойно зробили стратегію працездатною.
