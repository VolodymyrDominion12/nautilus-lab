...
> **Актуальний стан роботів і порогів гейта — у [STATUS.md](STATUS.md)** (генерується з коду й specs,
> ADR [0002](adr/0002-odna-heneruvana-tablytsia-stanu.md)). Таблиця нижче — історичний зріз на дату звірки.
...
| §10 Критерії приймання (DoD) | «ручний чекліст» | ✅ **машинний гейт** `application/promotion_gate.py`; пороги — лише в коді, їх показує [STATUS.md](STATUS.md) (ADR [0003](adr/0003-porohy-heita-z-kodu.md)). Ручними лишаються беззбитковість і стрес-слайси §10 |
...
```
                  Вхід нової ідеї (Гіпотеза / Специфікація)
                                     ...
                                     ▼
                ┌────────────────────────────────────────┐
                │  Walk-Forward (фолди за GateCriteria)  │
                └────────────────────┬───────────────────┘
                                     │
            ...
            │  PBO/DSR за GateCriteria    │ (Запис причини в specs)│
            └───────────┬──────────┘      └────────────────────────┘
                        │
                  Критерій виконано?
                        │
...
Числові пороги пунктів 1–4 не записуються тут: їх задає `GateCriteria` в
`application/promotion_gate.py`, а показує [STATUS.md](STATUS.md) (ADR
[0003](adr/0003-porohy-heita-z-kodu.md)). Колишні числа цього розділу (фолди ≥ 4,
PBO < 0.25) розходилися з кодом (6 і 0.3) — тепер джерело одне.

1. **Walk-Forward:** не менше `min_folds` послідовних ковзних фолдів з буфером-ембарго і не менше `min_oos_fills` OOS-угод.
...
3. **Ймовірність перенавчання (PBO):** PBO, розраховане через CSCV, не більше `max_pbo`.
4. **Дефльований коефіцієнт Шарпа (DSR):** DSR як імовірність не менше `min_dsr` (0.95 ≡ $p \le 0.05$), з урахуванням кількості протестованих конфігурацій сітки.
...
Розбіжність порогів §10 і `application/promotion_gate.py` розв'язано в E-2.8:
джерело правди — код, [STATUS.md](STATUS.md) — його відображення (ADR
[0003](adr/0003-porohy-heita-z-kodu.md)).
