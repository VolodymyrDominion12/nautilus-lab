...
| E-0.6 | `permissions: contents: read` у `ci.yml`; `Claude outputs/`, `models/` у `.gitignore` | 🟢 | ✅ `Claude outputs/` у `.gitignore`; ⏳ `permissions: contents: read`, `persist-credentials: false` — у патчі `e1-ci-supply-chain.patch` (замінює `ci-e0.patch`), `.github/`, `Makefile` і `.pre-commit-config.yaml` застосовує власник (захищені від запису інструментом) |
