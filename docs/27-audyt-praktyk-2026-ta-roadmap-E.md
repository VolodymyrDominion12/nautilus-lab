...
| E-2.2 | `JobManager` замість глобальних `CURRENT_*_PROCESS`; стан джобів у SQLite (`data/jobs.sqlite`), усиновлення/прибирання сиріт при старті | 🟠 | 🟡 крок 1 2026-09-25: `JobManager` (`api/jobs.py`) замість `CURRENT_*_PROCESS`/`JOBS_STARTING`/`global`: слот резервується в запиті, звільняється в `finally`, `schedule=` для тестів. ⏳ крок 2: стан у SQLite + усиновлення/прибирання сиріт при старті |
