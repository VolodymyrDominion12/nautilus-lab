...
| E-0.6 | `permissions: contents: read` у `ci.yml`; `Claude outputs/`, `models/` у `.gitignore` | 🟢 | ✅ `Claude outputs/` у `.gitignore`; ⏳ `permissions: contents: read` і `persist-credentials: false` — готовий патч, `.github/workflows/` застосовує власник вручну; `models/` лишено — рішення власника (моделі зараз, імовірно, версіонуються) |

### Хвиля 1 — довіра до paper-прогону (05.10 – 25.10.2026)

| ID | Задача | Зусилля | Definition of Done |
|---|---|:-:|---|
| E-1.1 | CI-джоба `frontend`: `npm ci`, `tsc -b`, `oxlint`, `vite build`, `check:logic` | 🟢 | блокуюча |
| E-1.2 | CI-джоба `docker`: збірка обох образів (без push), кеш buildx | 🟢 | блокуюча на PR, що чіпають `deploy/`, `pyproject.toml`, `frontend/` |
| E-1.3 | Ланцюг постачання: SHA-піни actions, Renovate (групи: python, npm, actions, docker; Nautilus — окремо), `pip-audit`/OSV, gitleaks, pre-commit | 🟠 | щотижневі PR від Renovate; CI падає на high-CVE і на секреті |
| E-1.4 | **`RunManifest`**: git SHA + dirty, хеш `uv.lock`, версія Nautilus, хеш налаштувань, відбиток каталогу (перелік файлів + розміри + mtime або sha256), seed-и; пишеться в кожен звіт, рядок журналу досліджень і `session_start` | 🟠 | `lab research` без чистого дерева друкує попередження; `live_paper_report.py` показує ревізію сесії |
| E-1.5 | `/healthz` (процес) і `/readyz` (кожен фід отримав бар не пізніше 2 × інтервал); Docker healthcheck → `/readyz` | 🟢 | контейнер `unhealthy`, коли фід завис |
| E-1.6 | Метрики Prometheus (`/metrics`, за токеном) + JSON-логи; алерт у Telegram на застарілий фід, помилку журналу, спрацювання breaker (це заодно закриває S3 для живого контуру) | 🟠 | алерт приходить протягом 1 бару після штучного обриву WS |
| E-1.7 | Бекап `data/paper/` щогодини (restic/rsync поза VPS) + dead-man's switch | 🟢 | відновлення з бекапу перевірено один раз вручну |
| E-1.8 | Хардинг compose: `read_only`, `tmpfs`, `cap_drop: [ALL]`, `no-new-privileges`, `mem_limit`; CSP/HSTS у Caddy; базові образи за digest (оновлює Renovate) | 🟢 | `docker inspect` підтверджує |
| E-1.9 | Покриття: прибрати `omit` для `backtest_runner`/`signal_strategy`/`instrument`, маркер `integration`; зробити крок `--cov` блокуючим | 🟠 | покриття виміряне **з** адаптерами; поріг — фактичне значення, а не 80 «зверху» |
...
| ID | Задача | Зусилля | Definition of Done |
|---|---|:-:|---|
| E-2.1 | Розбити `app.py` на `APIRouter`-и (`research`, `catalog`, `ml`, `paper`, `settings`, `status`) + `create_app(settings)` | 🟠 | `app.py` < 200 рядків; тести створюють app з фікстурних налаштувань без монкіпатчу модуля |
| E-2.2 | `JobManager` замість глобальних `CURRENT_*_PROCESS`; стан джобів у SQLite (`data/jobs.sqlite`), усиновлення/прибирання сиріт при старті | 🟠 | рестарт API не губить знання про запущену джобу |
| E-2.3 | Pydantic response-моделі для всіх маршрутів → `openapi-typescript` → `frontend/src/services/api.gen.ts`; CI перевіряє дрейф | 🟠 | ручні типи відповідей видалено |
...
| E-2.6 | «Золотий» регресійний бектест: фіксований синтетичний датасет + фіксований seed → знімок ключових метрик у репо; CI порівнює | 🟢 | оновлення Nautilus/pandas, що змінює результат, видно в PR |
| E-2.7 | Property-based тести (Hypothesis) для `marking`, `position_plan`, `walk_forward`, `risk`, `quantiles` | 🟠 | ≥ 1 властивість на модуль |
