...
- **Офлайн-контур ШІ-дослідження**: шаблони промптів у `research/prompts/`, артефакти гіпотез у `research/hypotheses/`, журнал рішень `research/journal.md` і скрипт `scripts/propose_alphas.py` (модель працює **поза** гарячим шляхом; ключ не потрапляє в артефакт). Див. [14](14-llm-model-u-torhivli.md) і [research/README.md](../research/README.md).
